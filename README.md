...
```powershell
.\scripts\Import-TestCases.ps1 -OrgUrl https://your-org.crm4.dynamics.com -PackPath .\my-pack.json
```
...
Headed Chromium opens, you log in interactively (incl. MFA), the resulting browser state is persisted as JSON. The `ui-setup` sub-command has a hard guard on the target environment: hosts containing `-dev.` or `-test.` are accepted; further non-production environments are opened only explicitly per call via `--allow-env <marker>` (repeatable, e.g. `--allow-env uat`); `prod`, `prd`, `production` and `live` are never accepted, and every other host is refused. For every environment other than DEV, a notice before the browser starts states that the project commissioning the tests governs which read and write steps are permitted there. The default output path is `auth/storage-state.json`. Storage-state files are short-lived (~24h for SPA flows) and must not be committed to source control.
