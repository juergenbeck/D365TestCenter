...
.DESCRIPTION
    ...
    flakiness (e.g. DEV orgs with long async plugin cascades).

    Designed to be called from a project-specific deploy script as a pre-deploy
    gate (e.g. before importing a managed solution to TEST):
    ...
    Retry behaviour:
      ...
      - Retry exists because plugin-cascade latency on DEV orgs can cause
...
.PARAMETER Filter
    ...
    Default: "*-UI-*" (matches UI smokes by the TestId convention <PROJECT>-UI-<n>).

.PARAMETER Config
    Cli config profile. Default: standard.
...
.EXAMPLE
    # Pre-deploy gate before importing a managed solution to TEST
    .\Run-UiSmokes.ps1 `
        -Org "https://contoso-dev.crm4.dynamics.com" `
        -ClientId "<client-id>" `
        -ClientSecret $env:D365TC_UISMOKE_CLIENT_SECRET `
        -TenantId "<tenant-id>" `
        -StorageState "C:\...\storage-state.json"
