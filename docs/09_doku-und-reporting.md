...
### 4a. Testdokumentation mit Belegbildern

Mit `run --evidence-dir <dir>` dokumentiert der Lauf sich selbst (ADR 2026-09-16-1957). Er schreibt
in den Ordner die Belegbilder, eine maschinenlesbare `evidence.json` und daraus zwei Dokumente mit
identischem Inhalt: `testdokumentation.html` (eine Datei, Bilder eingebettet, druckfähig) und
`testdokumentation.docx` (Word). Aufbau je Testfall:

1. Kopf: Titel, Beschreibung, Tags, Ergebnis, Dauer, Testlauf-Kennung.
2. Je dokumentiertem Schritt (`docSteps` des Testfalls, etwa die Schritte des Zephyr-Testskripts):
   Überschrift, Durchführung, erwartetes Ergebnis, Status.
3. Darunter die Belege dieses Schritts: Bildunterschrift (`caption`), eine Tabelle mit den zum
   Aufnahmezeitpunkt angezeigten Feldwerten (`highlightFields`) und die Bildausschnitte mit
   umrandeten Feldern.
4. Die ausgeführten technischen Schritte mit Status, Dauer und Rückmeldung (bei `evaluate` der
   zurückgegebene und der erwartete Wert).

Ein technischer Schritt gehört über `docStep` zu einem dokumentierten Schritt; Schritte ohne `docStep`
erben den zuletzt genannten, Schritte davor laufen unter „Vorbereitung“. Ein Testfall ohne `docSteps`
erscheint als ein Abschnitt „Ablauf“. Vor jeder Aufnahme blendet der Lauf bekannte Einblendungen der
Dynamics-Oberfläche per CSS aus (Copilot-Datensatzzusammenfassung, Knopf „Formularunterstützung“, Microsoft-Hinweisfenster wie „Formulare schneller ausfüllen mit KI“ oder „Anrufen ist jetzt noch einfacher!“);
es wird nichts angeklickt und nichts am Datensatz verändert. Felder und Optionen:
[Actions-Referenz, BrowserAction](handbuch/02-testfall-schreiben/02-actions-referenz.md#browseraction-nur-cli).
