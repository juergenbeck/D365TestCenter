...
Einen Testlauf starten heisst: einen `jbe_testrun`-Record anlegen mit
Status "Geplant". Das Speichern triggert das Plugin, der Lauf beginnt
automatisch.

## Schritt 1: Zu den Testlaeufen navigieren

In der App **D365 Test Center** im linken Menue: **Testlaeufe**.
...
**Name** — frei waehlbar. Gute Beispiele:
...
| Filter | Wirkung |
|---|---|
| (leer) oder `*` | Alle aktivierten Testfaelle |
...
- **Nicht angehakt** (Default): Testdaten werden nach dem Lauf geloescht.
- **Angehakt**: Testdaten bleiben — nuetzlich wenn du nach dem Lauf
...
angestossen, das CRUD-Trigger-Plugin `RunTestsOnStatusChange` ist aktiv
...
Cache kann es 2-3 Sekunden brauchen — notfalls **F5** druecken):
...
- **Batch mit N Tests** laeuft **sequenziell** (nicht parallel).
...
Gibt es **nicht**. Einmal gestartet laeuft der Lauf durch. Du kannst aber
den Testrun-Record loeschen, waehrend er laeuft — das beendet nicht das
Plugin im Hintergrund, aber der Record ist weg und du siehst kein Ergebnis
mehr. Nur fuer Notfaelle.

## Mehrere Testlaeufe gleichzeitig

Moeglich. Jeder Lauf ist isoliert durch `{TIMESTAMP}`. Die Plugin-Sandbox
kann ca. 5 gleichzeitige Ausfuehrungen pro Organisation verkraften. Mehr
