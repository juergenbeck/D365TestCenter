...
dotnet D365TestCenter.Cli.dll validate \
    --pack packs/lead-qualify-mapping.json
```
...
============================================================
  ...
  Pack:   projekt/packs/coverage.json
  Strict: False
============================================================

  Tests: 8

  [ERROR  ] PLG-UMSATZPLAN-01              Step 0    FILTER_FIELD_NOT_LOGICAL
             Filter field '_contoso_bestellungid_value' uses the OData lookup format. The internal
             QueryExpression builder expects the logical name (Singular, without '_..._value'
             wrapping).
             -> Use 'contoso_bestellungid' instead of '_contoso_bestellungid_value'.
...
```
[Test] PLG-UMSATZPLAN-01 -> FEHLER: Pre-Run-Validation hat 1 Error-Befund(e).
  Test wird nicht ausgefuehrt.
  Pre-run validation failed with 1 error(s):
    Step 0 FILTER_FIELD_NOT_LOGICAL: Filter field '_contoso_bestellungid_value' uses
