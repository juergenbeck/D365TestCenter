...
| [`BrowserAction`](#browseraction-nur-cli) | Oberfläche bedienen und prüfen, Belegbilder aufnehmen (nur CLI mit `--browser-state`) |
...
{ "stepNumber": 4, "action": "ExecuteRequest",
  "requestName": "contoso_RecalculateScore",
...
{ "action": "SetEnvironmentVariable",
  "schemaName": "contoso_RetentionDays",
  "value": "1",
  "target": "effective",
  "alias": "envSnap" }
```
...
[
  { "action": "SetEnvironmentVariable",
    "schemaName": "contoso_feature_enabled",
    "value": "false",
    "alias": "envSnap" },
  { "action": "CreateRecord", "entity": "contacts", "alias": "c", "fields": {} },
  { "action": "UpdateRecord", "alias": "c",
    "fields": { "contoso_gdprstatuscode": 288260001 } }
]
```

Nach dem Test wird `contoso_feature_enabled` automatisch auf
...
[
  { "action": "RetrieveEnvironmentVariable",
    "schemaName": "contoso_max_retries", "alias": "envMax" },
...
## BrowserAction (nur CLI)

Bedient die Model-Driven-Oberfläche über Playwright (ADR-0006). Läuft nur im CLI-Pfad mit
`run --browser-state`; im Plugin-Pfad und ohne den Schalter wird der Step übersprungen. Operationen
(`operation`): `navigate` (`url`), `click` / `doubleClick` (`selector`, `fallbackSelector`), `fill`
(`selector`, `value`), `waitFor` (`selector`), `delay` (`delayMs`), `evaluate` (`expression`, optional
`value` als Gleichheitsprüfung, `outputAlias`) und `screenshot`.

`evaluate` endet nach `timeoutSeconds` (Standard 120) mit einem Fehler samt Seitenzustand (Adresse, Anzahl
Frames, offene Dialoge, ob die Seite noch antwortet), statt den Lauf anzuhalten; ein Skript, das selbst
länger wartet, braucht einen höheren Wert. Ist `value` gesetzt und liefert das Hauptfenster einen anderen Wert,
wird der Ausdruck in jedem iFrame wiederholt (je Frame höchstens 10 Sekunden), damit Bibliotheken in einem
eingebetteten Frame gefunden werden. `frameFallback: false` schaltet das ab; das ist für Skripte mit
Nebenwirkungen oder langen Wartezeiten richtig, weil sie sonst je Frame erneut laufen.
Native Browserdialoge werden protokolliert (`browser-dialog`) und geschlossen.

### Belegbilder mit `screenshot`

```json
{ "stepNumber": 17, "action": "BrowserAction", "operation": "screenshot",
  "name": "lead-ohne-kontakt",
  "caption": "Genau ein Lead: Übergeordneter Kontakt leer, Leistungsnutzung verweist auf die Buchung",
  "highlightFields": ["subject", "parentcontactid", "contoso_productusageid"] }
```

| Feld | Default | Wirkung |
|---|---|---|
| `name` | Zeitstempel | Teil des Dateinamens |
| `caption` | `description` | Was das Bild belegen soll; Bildunterschrift in der Testdokumentation |
| `highlightFields` | - | Logische Feldnamen des Formulars. Die Felder werden umrandet, ihre Beschriftung und ihr angezeigter Wert werden über die Formular-API gelesen und neben dem Bild ausgegeben. Ein Feld, das nicht auf dem Formular sichtbar ist, erzeugt einen Hinweis, keinen Fehler. Auch der Name einer Unterliste (Subgrid-Control, etwa `Subgrid_new_6`) ist zulässig: sie wird umrandet, und als Istwert steht die Zahl ihrer Datensätze; ihr Reiter muss vorher geöffnet und die Liste geladen sein (z. B. per `evaluate` mit `getParent().getParent().setFocus()`). |
| `clip` | `section` mit `highlightFields`, sonst `page` | `section`: nur die Formularabschnitte der Felder, nah beieinanderliegende Abschnitte in einem Bild; `viewport`: sichtbarer Bereich; `page`: ganze Seite |
| `hideOverlays` | `true` | Bekannte Einblendungen vor der Aufnahme per CSS ausblenden, danach wiederherstellen |
| `hideSelectors` | - | Weitere CSS-Selektoren, die für die Aufnahme ausgeblendet werden |

Ohne `run --evidence-dir` und ohne diese Felder bleibt das bisherige Verhalten: ganzseitiges PNG
nach `%TEMP%\<name>.png`.

### Dokumentierte Schritte: `docSteps` und `docStep`

Der Testfall kann seinen manuellen Ablauf mitführen; jeder technische Step verweist mit `docStep` auf
einen dieser Schritte (folgende Steps ohne `docStep` erben ihn):

```json
"docSteps": [
  { "number": 1, "title": "Firma anlegen", "action": "Eine neue Firma anlegen ...", "expected": "Die Firma ist gespeichert." }
],
"steps": [
  { "stepNumber": 1, "docStep": 1, "action": "BrowserAction", "operation": "navigate", "url": "..." }
]
```

Die Testdokumentation gruppiert danach Belege und technische Schritte, siehe
[Doku und Reporting](../../09_doku-und-reporting.md#4a-testdokumentation-mit-belegbildern).
