...
| `jbe_name` | String (Primary) | Anzeigename. Standardmaessig gleich `title` aus dem JSON. |
| `jbe_title` | String | Aussagekraeftiger Titel des Tests. |
| `jbe_tags` | String | Komma-getrennte Tags zur Kategorisierung. |
| `jbe_userstories` | String | Jira/DevOps-Ticket-Referenzen. |
| `jbe_enabled` | Two Options | `Ja`/`Nein`. Deaktivierte Tests werden beim Run uebersprungen. |
| `jbe_definitionjson` | Mehrzeilentext | **Hier steht die komplette JSON-Definition.** |

Das Feld `jbe_definitionjson` ist das Herzstueck. Alles was der Test tun
...
| `jbe_keeprecords` | Two Options | `Ja` behaelt Testdaten, `Nein` raeumt am Ende auf. |
...
| `jbe_testrunid` | Lookup | Rueckverweis auf den `jbe_testrun`. |
...
- **Error** — eine Action hat geworfen (Netzwerkfehler, ungueltiger Alias,
...
Fuer jede Action im Test wird ein Step-Record geschrieben. Das ist dein
Log zum Mitlesen.
...
| Feld | Typ | Bedeutung |
|---|---|---|
| `jbe_stepnumber` | Integer | Reihenfolge (1, 2, 3, ..., meist auch 9000 fuer Cleanup). |
...
| `jbe_assertionfield` | String | Bei Assert: geprueftes Feld. |
| `jbe_expectedvalue` | String | Bei Assert: erwarteter Wert. |
| `jbe_actualvalue` | String | Bei Assert: tatsaechlicher Wert. |
| `jbe_recordid` | String | ID des erzeugten/bearbeiteten Records (als Klick-Link). |
| `jbe_inputdata` / `jbe_outputdata` | Mehrzeilentext | Rohdaten fuer Debugging. |
| `jbe_testrunresultid` | Lookup | Rueckverweis. |
...
Standardmaessig loescht das Test Center am Ende des Laufs alle Records,
die waehrend des Tests angelegt wurden (per interner Record-Tracker-Liste).
Ausnahmen:

- `jbe_keeprecords = Ja` im Testrun: **nichts** wird geloescht, du siehst
  alle Testdaten in Dataverse. Nuetzlich zum Debugging.
- Einzelne `CreateRecord`-Steps mit `"track": false`: werden nicht fuers
  Cleanup erfasst (selten gebraucht).

Die Test-Records selbst (`jbe_testrun`, `jbe_testrunresult`, `jbe_teststep`)
werden **nie** automatisch geloescht — sie sind dein Audit-Log.
