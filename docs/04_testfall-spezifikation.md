...
| `preconditions.existingContactSources[].alias` | String | Pflicht | Alias-Name der ContactSource (z.B. `"source1"`) |
| `preconditions.existingContactSources[].sourceSystem` | Integer | Pflicht | OptionSet-Wert des Quellsystems (projektspezifisch) |
...
| `entity` | String | Pflicht | Logischer Entity-Name (z.B. `"account"`, `"lead"`, `"contoso_customentity"`) |
...
| ContactSource | `ContactSource:<alias>` | Prüft ein Feld einer spezifischen ContactSource (z.B. `ContactSource:source1`) |
| AuditLog | `AuditLog` | Prüft Felder im Audit Logging (z.B. `contoso_decisions`) |
