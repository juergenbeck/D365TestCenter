...
| `--config <profil>` | Konfigurationsprofil: nur noch `standard` (Default). Frühere Profilnamen werden weiter angenommen, mit einem Hinweis auf `standard` abgebildet und verhalten sich identisch. |
...
run --org <url> <auth> [--filter <pattern>] [--keep-records] [--config <profil>]
    ...
    [--sync-defs <dir>] [--env <label>] [--evidence-dir <dir>]
```
...
| `--evidence-dir` | - | Testdokumentation des Laufs in diesem Ordner: Belegbilder (`bilder/`), `evidence.json`, `testdokumentation.html` und `testdokumentation.docx`. Ohne den Schalter entsteht kein Ordner, und `screenshot`-Steps ohne die neuen Felder schreiben wie bisher nach `%TEMP%`. Siehe [Doku und Reporting](09_doku-und-reporting.md#4a-testdokumentation-mit-belegbildern). |
...
`https://contoso-dev.crm4.dynamics.com`). Weitere Nicht-Produktionsumgebungen werden nur ausdrücklich je
Aufruf per `--allow-env <marker>` geöffnet (wiederholbar, z.B. `--allow-env uat` für
`contoso-uat.crm4.dynamics.com`). Die Marker `prod`, `prd`, `production` und `live` werden auch über
`--allow-env` nie angenommen. Maßgeblich ist nur der Host; jede sonstige Adresse wird mit Exit-Code 2
abgewiesen. Bei jeder Umgebung außer DEV erscheint vor dem Browserstart der Hinweis, dass das Projekt,
das die Tests beauftragt, regelt, welche Lese- und Schreibschritte dort zulässig sind; der
Anmeldezustand erweitert den beauftragten Testumfang nicht.

```
ui-setup --org <url> [--output <pfad>] [--allow-env <marker>]...
```

| Option | Default | Beschreibung |
|---|---|---|
| `--output` | `auth/storage-state.json` | Ausgabepfad des Storage-State-JSON (in `.gitignore` halten) |
| `--allow-env` | - | Zusätzlicher Marker einer Nicht-Produktionsumgebung (nur Buchstaben und Ziffern, z.B. `uat` für Hosts mit `-uat.`); wiederholbar |
...
### `evidence-report`: Testdokumentation neu erzeugen

Erzeugt `testdokumentation.html` und `testdokumentation.docx` erneut aus der `evidence.json` eines Laufs,
der mit `run --evidence-dir` gelaufen ist. Kein Dataverse-Zugriff, keine Anmeldung. Nützlich nach einer
Änderung am Layout: dieselbe `evidence.json` ergibt eine bytegleiche HTML-Datei.

```
evidence-report --evidence-dir <dir>
```

Exit-Code `2`, wenn `evidence.json` fehlt oder ungültig ist.
