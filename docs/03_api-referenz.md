...
const newRun = await API.create("jbe_testruns", {
    jbe_teststatus: 105710000,     // Ausstehend
...
|---------------------------------|----------|----------------------|-----------------------------------------------------------|
| `prefix`                       | `string` | `"jbe"`              | Publisher-Prefix (ohne Unterstrich)                       |
| `optionSetBase`                | `number` | `105710000`          | Basiswert für OptionSet-Codes                             |
| **entities**                   |          |                      |                                                           |
...
| **optionSets**                 |          |                      |                                                           |
| `optionSets.statusPlanned`    | `number` | `105710000`          | Teststatus: Ausstehend (Pending)                          |
| `optionSets.statusRunning`    | `number` | `105710001`          | Teststatus: Läuft                                         |
| `optionSets.statusCompleted`  | `number` | `105710002`          | Teststatus: Abgeschlossen                                 |
| `optionSets.statusError`      | `number` | `105710003`          | Teststatus: Fehler                                        |
| `optionSets.statusSplitting`  | `number` | `105710004`          | Teststatus: Aufteilung läuft (Worker-Modell)              |
| `optionSets.chunkNew`         | `number` | `105710000`          | Chunk-Status: Neu                                         |
| `optionSets.chunkRunning`     | `number` | `105710001`          | Chunk-Status: Läuft                                       |
| `optionSets.chunkResume`      | `number` | `105710002`          | Chunk-Status: Fortsetzen                                  |
| `optionSets.chunkProcessed`   | `number` | `105710003`          | Chunk-Status: Verarbeitet                                 |
| `optionSets.chunkError`       | `number` | `105710004`          | Chunk-Status: Fehler                                      |
| `optionSets.outcomePassed`    | `number` | `105710000`          | Testergebnis: Passed                                      |
| `optionSets.outcomeFailed`    | `number` | `105710001`          | Testergebnis: Failed                                      |
| `optionSets.outcomeSkipped`   | `number` | `105710002`          | Testergebnis: Skipped                                     |
| `optionSets.outcomeError`     | `number` | `105710003`          | Testergebnis: Error                                       |
| `optionSets.outcomeNotImpl`   | `number` | `105710004`          | Nur UI-Konstante, im OptionSet `jbe_testoutcome` nicht vorhanden |
| `optionSets.catUpdateSource` | `number` | `105710000` | Kategorie: Update Source |
| `optionSets.catCreateSource` | `number` | `105710001` | Kategorie: Create Source |
| `optionSets.catDeleteSource` | `number` | `105710002` | Kategorie: Delete Source |
| `optionSets.catMultiSource` | `number` | `105710003` | Kategorie: Multi-Source |
| `optionSets.catMerge` | `number` | `105710004` | Kategorie: Merge |
| `optionSets.catCustomApi` | `number` | `105710005` | Kategorie: Custom API |
| `optionSets.catConfig` | `number` | `105710006` | Kategorie: Config |
| `optionSets.catEndToEnd` | `number` | `105710007` | Kategorie: End-to-End |
| `optionSets.catErrorHandling` | `number` | `105710008` | Kategorie: Error Handling |
| **actions**                    |          |                      |                                                           |
...
Um das Integration Test Center mit einem anderen Publisher-Prefix zu verwenden (z.B. `contoso` statt `jbe`):

1. **CONFIG.prefix ändern:** `prefix: "contoso"` setzen
2. **Alle Entity-Namen anpassen:** `entities.testcase` wird zu `"contoso_testcase"` usw.
3. **Alle Feldnamen anpassen:** Jeder Eintrag unter `fields` muss das neue Prefix erhalten
4. **OptionSet-Namen anpassen:** Die OptionSets heißen dann `contoso_teststatus`, `contoso_testoutcome`, `contoso_testcategory`
5. **Custom Action anpassen:** `actions.runTests` wird zu `"contoso_RunIntegrationTests"`
6. **OptionSet-Werte beibehalten:** Die numerischen Codes (105710000 ff.) bleiben gleich, sofern die OptionSets mit denselben Werten angelegt werden
...
1. Client erstellt einen `jbe_testrun`-Datensatz mit Status `Ausstehend` (105710000) und dem gewünschten Filter
2. Client ruft `jbe_RunIntegrationTests` mit der Testlauf-GUID auf
3. Serverseitig wird der Status auf `Läuft` (105710001) gesetzt
...
6. Nach Abschluss wird der Testlauf-Status auf `Abgeschlossen` (105710002) oder `Fehler` (105710003) gesetzt
...
const poll = setInterval(async () => {
    ...
    // Abgeschlossen (105710002) oder Fehler (105710003); 105710004 = Aufteilung läuft, noch nicht fertig
    if (status.jbe_teststatus === 105710002 || status.jbe_teststatus === 105710003) {
        clearInterval(poll);
