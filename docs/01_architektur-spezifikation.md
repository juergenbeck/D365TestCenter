...
**Properties:**
- `prefix` (String): Publisher-Prefix, Standard `"jbe"`
- `optionSetBase` (Number): Basiswert für OptionSets, Standard `105710000`
...
- `optionSets` (Object): Numerische Werte für TestStatus (Planned/Running/Completed/Error/Splitting), ChunkStatus (New/Running/Resume/Processed/Error), Outcome (Passed/Failed/Skipped/Error, dazu die reine UI-Konstante NotImpl), Category (UpdateSource bis ErrorHandling)
...
- `API.getMany("jbe_testrunresults", ...)`: Fehlgeschlagene Tests (outcome eq 105710001), letzte 10
...
- Filter auf Status (Ausstehend, Läuft, Abgeschlossen, Fehler, Aufteilung läuft)
...
Der CONFIG-Block (im `<script>`-Teil von `webresource/d365testcenter.html`) definiert alle anpassbaren Schema-Namen:

| Property                     | Wert (Standard)             | Beschreibung                               |
|------------------------------|-----------------------------|--------------------------------------------|
| `prefix`                     | `"jbe"`                     | Publisher-Prefix (ohne Unterstrich)        |
| `optionSetBase`              | `105710000`                 | Basiswert für alle Custom OptionSets       |
...
| `optionSets.statusPlanned`   | `105710000`                 | Status: Ausstehend (Pending)               |
| `optionSets.statusRunning`   | `105710001`                 | Status: Läuft                              |
| `optionSets.statusCompleted` | `105710002`                 | Status: Abgeschlossen                      |
| `optionSets.statusError`     | `105710003`                 | Status: Fehler                             |
| `optionSets.statusSplitting` | `105710004`                 | Status: Aufteilung läuft (Worker-Modell)   |
| `optionSets.chunkNew`        | `105710000`                 | Chunk-Status: Neu                          |
| `optionSets.chunkRunning`    | `105710001`                 | Chunk-Status: Läuft                        |
| `optionSets.chunkResume`     | `105710002`                 | Chunk-Status: Fortsetzen                   |
| `optionSets.chunkProcessed`  | `105710003`                 | Chunk-Status: Verarbeitet                  |
| `optionSets.chunkError`      | `105710004`                 | Chunk-Status: Fehler                       |
| `optionSets.outcomePassed`   | `105710000`                 | Ergebnis: Passed                           |
| `optionSets.outcomeFailed`   | `105710001`                 | Ergebnis: Failed                           |
| `optionSets.outcomeSkipped`  | `105710002`                 | Ergebnis: Skipped                          |
| `optionSets.outcomeError`    | `105710003`                 | Ergebnis: Error                            |
| `optionSets.outcomeNotImpl`  | `105710004`                 | Nur UI-Konstante, im OptionSet `jbe_testoutcome` nicht vorhanden |
| `optionSets.catUpdateSource` | `105710000` | Kategorie: Update Source |
| `optionSets.catCreateSource` | `105710001` | Kategorie: Create Source |
| `optionSets.catDeleteSource` | `105710002` | Kategorie: Delete Source |
| `optionSets.catMultiSource` | `105710003` | Kategorie: Multi-Source |
| `optionSets.catMerge` | `105710004` | Kategorie: Merge |
| `optionSets.catCustomApi` | `105710005` | Kategorie: Custom API |
| `optionSets.catConfig` | `105710006` | Kategorie: Config |
| `optionSets.catEndToEnd` | `105710007` | Kategorie: End-to-End |
| `optionSets.catErrorHandling` | `105710008` | Kategorie: Error Handling |

### Prefix ändern

Um den Publisher-Prefix zu ändern (z.B. von `jbe` auf `contoso`):

1. `CONFIG.prefix` anpassen
2. Alle Werte in `CONFIG.entities` und `CONFIG.fields` anpassen (z.B. `jbe_testcase` zu `contoso_testcase`)
...
Alle globalen OptionSets der Solution nutzen den Wertebereich ab `105710000` (festgelegt in
`solution/src/OptionSets/*.xml`, gespiegelt in `CONFIG.optionSets`):

- `jbe_teststatus`: 105710000 bis 105710004 (Ausstehend, Läuft, Abgeschlossen, Fehler, Aufteilung läuft)
- `jbe_chunkstatus`: 105710000 bis 105710004 (Neu, Läuft, Fortsetzen, Verarbeitet, Fehler)
- `jbe_testoutcome`: 105710000 bis 105710003 (Passed, Failed, Skipped, Error)
- `jbe_stepstatus`: 105710000 bis 105710002 (Success, Failed, Skipped)
- `jbe_testcategory`: 105710000 bis 105710008
- `jbe_lifecyclestatus`: 105710000 bis 105710004 (Entwurf, Aktiv, Instabil, Historisch, Archiviert)

Bei `jbe_testoutcome` ist Skipped `105710002` und Error `105710003`; ältere Stände der Doku und der UI
hatten die beiden vertauscht. `CONFIG.optionSets.outcomeNotImpl` (`105710004`) hat im OptionSet keine
Entsprechung. Die Werte in `CONFIG` müssen exakt mit den in Dataverse definierten OptionSet-Werten
übereinstimmen.
...
auf `CONFIG.optionSetBase` ab (`100000000 + i` wird `105710000 + i`).
