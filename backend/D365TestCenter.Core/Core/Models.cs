...
public sealed class TestCase
{
    ...
    // ── Test documentation (ADR 2026-09-16-1957) ──────────────

    /// <summary>
    /// Optional documented (usually manual) procedure of this test case, e.g. the
    /// steps of a Zephyr test script. Technical steps link to it via
    /// <see cref="TestStep.DocStep"/>; the evidence documentation groups the
    /// executed steps and their screenshots under these entries.
    /// </summary>
    [JsonProperty("docSteps")]
    public List<DocStepDefinition>? DocSteps { get; set; }
...
/// <summary>
/// One documented step of a test case (ADR 2026-09-16-1957), typically taken from the
/// manual test script: what is done and what is expected.
/// </summary>
public sealed class DocStepDefinition
{
    [JsonProperty("number")]
    public int Number { get; set; }

    /// <summary>Optional short title for the heading ("Schritt 3: Auftrag anlegen").</summary>
    [JsonProperty("title")]
    public string? Title { get; set; }

    /// <summary>What is done in this step (plain text, line breaks allowed).</summary>
    [JsonProperty("action")]
...
public sealed class TestStep
{
    ...
    // ── Evidence screenshots (ADR 2026-09-16-1957) ─────────────────────────

    /// <summary>BrowserAction screenshot: what the image is meant to prove (caption in the documentation).</summary>
    [JsonProperty("caption")]
    public string? Caption { get; set; }

    /// <summary>
    /// BrowserAction screenshot: logical names of model-driven form fields. They are
    /// outlined in the image, and their label plus display value are read via the form
    /// API at capture time and stored as actual values next to the image.
    /// </summary>
    [JsonProperty("highlightFields")]
    public List<string>? HighlightFields { get; set; }

    /// <summary>
    /// BrowserAction screenshot: image area. <c>section</c> (default when
    /// highlightFields are set): one image part per form section containing a
    /// highlighted field; <c>viewport</c>: visible area; <c>page</c>: full page
    /// (default otherwise, previous behaviour).
    /// </summary>
    [JsonProperty("clip")]
    public string? Clip { get; set; }

    /// <summary>
    /// BrowserAction screenshot: hide known Dynamics 365 overlays (e.g. the Copilot
    /// record summary) before capturing. Default true. Hiding is CSS only; nothing is
    /// clicked and the record is not changed.
    /// </summary>
    [JsonProperty("hideOverlays")]
    public bool? HideOverlays { get; set; }

    /// <summary>BrowserAction screenshot: additional CSS selectors to hide before capturing.</summary>
    [JsonProperty("hideSelectors")]
    public List<string>? HideSelectors { get; set; }

    /// <summary>
    /// BrowserAction evaluate with <c>value</c>: when the top frame returns a different
    /// value, re-run the expression in each sub-frame (default true, the original
    /// behaviour for libraries hosted in an iframe). Set false for scripts with side
    /// effects or long waits, which would otherwise repeat once per frame.
    /// </summary>
    [JsonProperty("frameFallback")]
    public bool? FrameFallback { get; set; }

    /// <summary>
    /// Number of the documented step (<see cref="TestCase.DocSteps"/>) this step belongs
    /// to. Steps without a value belong to the most recently numbered documented step;
    /// steps before the first numbered one count as preparation. 0 means preparation.
    /// </summary>
    ...
    /// Monatszeilen an einer Test-Auftragsposition). Gültig auf CreateRecord sowie (nur in
    ...
    /// "QualifyLead", "contoso_RecalculateScore").
    /// </summary>
    ...
    /// <summary>
    /// ExecuteRequest-Output als Alias verfügbar machen (A4 / Kunden-Feedback).
    ...
    /// Records. Korrelation läuft über regardingobjectid (Plattform-Befund auf einer DEV-Org 2026-06-28:
...
/// Größe ab (z.B. contoso_umsatzplan-Monatszeilen an einer Test-Auftragsposition, Restrict-Delete),
...
{
    /// <summary>Linker Vergleichswert (Platzhalter erlaubt, z.B. "{wbcfg.fields.contoso_writebacktocontact}").</summary>
    [JsonProperty("left")]
...
public sealed class TestContext
{
    ...
    /// contoso_fieldconfig) auf der Ziel-Umgebung. Ein gefundener Record ist kein erzeugter.
    /// </summary>
...
{
    /// <summary>
    /// Id of the jbe_testrun record this result belongs to (set by the orchestrator;
    /// null when the runner was used without persistence).
    /// </summary>
    [JsonProperty("testRunId", NullValueHandling = NullValueHandling.Ignore)]
...
public sealed class TestCaseResult
{
    ...
    /// <summary>
    /// B5 / Kunden-Feedback: alle vom Test angelegten Records (Alias plus
