...
public class MarkdownToHtmlTests
{
    ...
    {
        var html = MarkdownToHtml.Convert("Das ist **wichtig** und `contoso_adresse` hier.");
        Assert.Contains("<p>", html);
        Assert.Contains("<strong>wichtig</strong>", html);
        Assert.Contains("<code>contoso_adresse</code>", html);
    }

    [Fact]
    public void Code_WithUnderscores_NotItalic()
    {
        var html = MarkdownToHtml.Convert("`contoso_isderivingaddress1fromaccount` aktiv");
        Assert.Contains("<code>contoso_isderivingaddress1fromaccount</code>", html);
