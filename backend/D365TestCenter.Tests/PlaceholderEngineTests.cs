...
public class PlaceholderEngineTests
{
    ...
    public void Resolve_RecordAlias_FromRecordsRegistry()
    {
        ...
        ctx.Records["bridge1"] = ("contoso_bridge_pf_record", recordId);
    ...
    {
        var ctx = CreateContext();
        var entity = new Entity("contoso_fg_contactsource", Guid.NewGuid());
        entity["contoso_firstname"] = "MaxTest";
        ctx.FoundRecords["foundCS"] = entity;

        var result = _engine.Resolve("{RESULT:foundCS.contoso_firstname}", ctx);
        Assert.Equal("MaxTest", result);
    }
    ...
    public void Resolve_Row_ReplacesFromDataRow()
    {
        ...
        Assert.Equal("Source=4, Name=AlphaTest", result);
    }
    ...
    public void ResolveAll_ReplacesAllStringValues()
    {
        ...
        {
            ["contoso_firstname"] = "Test_{TESTID}",
            ["contoso_status"] = 1,
            ["contoso_externalid"] = "{GENERATED:ext}"
        };

        var resolved = _engine.ResolveAll(fields, ctx);

        Assert.Equal("Test_TC99", resolved["contoso_firstname"]);
        Assert.Equal(1, resolved["contoso_status"]); // int bleibt unverändert
        Assert.Equal(8, ((string)resolved["contoso_externalid"]!).Length);
    }
