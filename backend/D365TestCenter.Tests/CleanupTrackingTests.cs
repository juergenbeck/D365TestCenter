...
public class CleanupTrackingTests
{
    ...
    public void CleanupChildren_DeletesDeclaredChildrenBeforeParent()
    {
        ...
        var result = runner.RunAll(new List<TestCase>
        {
            new()
            {
                ...
                Steps = new List<TestStep>
                {
                    // Simuliert die Test-Auftragsposition, an der ein Plugin asynchron Monatszeilen
                    // (Restrict-Delete-Kinder) erzeugt hat: ohne die Deklaration
                    // scheitert der Cleanup-Delete des Parents dauerhaft.
                    new()
                    {
                        StepNumber = 1, Action = "CreateRecord", Entity = "contoso_bestellung", Alias = "pos",
                        CleanupChildren = new List<CleanupChildRelation>
                        {
                            new() { Entity = "contoso_umsatzplan", LookupField = "contoso_bestellungid" }
                        }
                    }
                }
            }
    ...
    {
        // Live belegt (DEV-Org): der async Abbau-Job eines Parent-Deletes löscht
        // dieselben contoso_umsatzplan-Kinder parallel -> "More than one concurrent Delete
    ...
    private sealed class RestrictCleanupService : IOrganizationService
    {
        ...
        public void Delete(string entityName, Guid id)
        {
            if (entityName == "contoso_umsatzplan")
            {
        ...
        public EntityCollection RetrieveMultiple(QueryBase query)
        {
            ...
            if (query is QueryExpression qe && qe.EntityName == "contoso_umsatzplan")
                foreach (var childId in LivingChildIds)
                    ec.Entities.Add(new Entity("contoso_umsatzplan", childId));
            return ec;
        }
