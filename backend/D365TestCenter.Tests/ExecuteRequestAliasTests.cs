...
public class ExecuteRequestAliasTests
{
    ...
    public void TestStep_ApiName_DeserializesFromJson()
    {
        ...
        Assert.NotNull(step);
        Assert.Equal("contoso_CancelInvoice", step!.ApiName);
        Assert.NotNull(step.Parameters);
    ...
    {
        // Legacy CallCustomApi packs (e.g., older project coverage packs) must keep working.
        ...
        Assert.Equal("contoso_DoSomething", svc.LastRequest!.RequestName);
    ...
    {
        // Pattern used by existing project packs.
        var (svc, runner) = NewRunner();
        ...
        var test = MakeTest(new TestStep
        {
            StepNumber = 1,
            Action = "ExecuteAction",
            ApiName = "contoso_RunFieldGovernanceForContact",
        ...
        Assert.NotNull(svc.LastRequest);
        Assert.Equal("contoso_RunFieldGovernanceForContact", svc.LastRequest!.RequestName);
    ...
    {
        // Mixed form seen in project packs: ExecuteAction verb, entity+fields schema.
        var (svc, runner) = NewRunner();
        ...
        var test = MakeTest(new TestStep
        {
            StepNumber = 1,
            Action = "ExecuteAction",
            Entity = "contoso_RunFieldGovernanceForContact",
