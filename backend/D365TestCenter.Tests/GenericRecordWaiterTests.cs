...
public class GenericRecordWaiterTests
{
    ...
    {
        // contoso_recordid ist ein String-Feld, das GUIDs als Strings speichert.
        ...
        var cache = EntityMetadataCache.CreateForTesting(
            ...
            {
                ["contoso_gdprpseudonymmap"] = new()
                {
                    ["contoso_recordid"] = AttributeTypeCode.String
                }
            });

        var filters = new List<FilterCondition>
        {
            new() { Field = "contoso_recordid", Operator = "eq",
                    Value = "12345678-1234-1234-1234-123456789012" }
        };

        var query = GenericRecordWaiter.BuildQuery(
            "contoso_gdprpseudonymmap", filters, null,
            metadataCache: cache);
