...
///
/// Bug (2026-05-18): EvaluateQueryAssertion und
...
public class AssertionEngineEntityResolveTests
{
    ...
    public void QueryAssertion_PluralEntityName_WithCache_ResolvesToSingular()
    {
        ...
        var cache = CacheWith("contoso_gdprpseudonymmap", "contoso_originalvalue");
        var engine = new AssertionEngine(cache);

        var assertion = new TestAssertion
        {
            Target = "Query",
            Entity = "contoso_gdprpseudonymmaps",  // Plural-EntitySetName
            Filter = new List<FilterCondition>
            {
                new FilterCondition { Field = "contoso_originalvalue", Operator = "eq", Value = "abc" }
            },
            Operator = "Exists"
        };
        ...
        Assert.Equal("contoso_gdprpseudonymmap", svc.RetrieveMultipleEntityNames[0]);
    }
    ...
    {
        var svc = new RecordingService();
        var cache = CacheWith("contoso_gdprpseudonymmap", "contoso_field");
        var engine = new AssertionEngine(cache);

        var assertion = new TestAssertion
        {
            Target = "Query",
            Entity = "contoso_gdprpseudonymmap",
            Filter = new List<FilterCondition>
            {
                new FilterCondition { Field = "contoso_field", Operator = "eq", Value = "x" }
            },
            Operator = "Exists"
        };
    ...
    public void QueryAssertion_WithoutCache_EntityNameUnchanged()
    {
        ...
        {
            Target = "Query",
            Entity = "contoso_gdprpseudonymmaps",
            Filter = new List<FilterCondition>
            {
                new FilterCondition { Field = "contoso_field", Operator = "eq", Value = "x" }
            },
        ...
        Assert.Equal("contoso_gdprpseudonymmaps", svc.RetrieveMultipleEntityNames[0]);
    }
    ...
    public void RecordAssertion_ExplicitPluralEntity_WithCache_ResolvesToSingular()
    {
        ...
        var id = Guid.NewGuid();
        ctx.RegisterRecord("alias1", "contoso_gdprpseudonymmap", id);

        var assertion = new TestAssertion
        {
            Target = "Record",
            RecordRef = "alias1",
            Entity = "contoso_gdprpseudonymmaps",  // Plural explizit gesetzt
            Field = "contoso_field",
            Operator = "IsNotNull"
        };
        ...
        Assert.Equal("contoso_gdprpseudonymmap", svc.RetrieveCalls[0].entityName);
    }
}
