...
public class PackValidatorTests
{
    ...
    public void FilterField_ODataFormat_IsError()
    {
        var tc = TestCaseWith(new TestStep
        {
            ...
            {
                new FilterCondition { Field = "_contoso_contactid_value", Operator = "eq", Value = "{c.id}" }
            }
        });
        var report = new PackValidator().ValidateOne(tc);

        var finding = AssertSingle(report, "FILTER_FIELD_NOT_LOGICAL");
        Assert.Contains("contoso_contactid", finding.Suggestion);
    }
    ...
    public void FilterField_LogicalName_Ok()
    {
        var tc = TestCaseWith(new TestStep
        {
            ...
            Filter = new List<FilterCondition>
            {
                new FilterCondition { Field = "contoso_contactid", Operator = "eq", Value = "{c.id}" }
            }
        });
        var report = new PackValidator().ValidateOne(tc);
    ...
    public void UnknownStepKey_CommentAnnotation_NotFlagged()
    {
        ...
        // empirically used in project UI packs. Must not be flagged.
        const string json = """
        {
    ...
    public void TestRunner_PreRunValidation_AbortsOnError()
    {
        ...
        var bad = TestCaseWith(new TestStep
        {
            ...
            Filter = new List<FilterCondition>
            {
                new FilterCondition { Field = "_contoso_contactid_value", Operator = "eq", Value = "x" }
            }
        });
    ...
    public void Condition_UndefinedAlias_FlaggedByAliasUndefined()
    {
        ...
        var tc = TestCaseWith(ConditionalWait(new StepCondition
        {
            Left = "{wbcfg.fields.contoso_writebacktocontact}", Operator = "Equals", Right = "true"
        }));
        var finding = AssertSingle(new PackValidator().ValidateOne(tc), "ALIAS_UNDEFINED");
    ...
    public void Condition_AliasDefinedBeforeUse_NoFinding()
    {
        ...
        var tc = TestCaseWith(
            new TestStep { StepNumber = 1, Action = "FindRecord", Entity = "contoso_fg_fieldconfigs", Alias = "wbcfg",
                Filter = new List<FilterCondition> { new FilterCondition { Field = "contoso_name", Operator = "eq", Value = "x" } } },
            new TestStep { StepNumber = 2, Action = "Wait", WaitSeconds = 1,
                Condition = new StepCondition { Left = "{wbcfg.fields.contoso_writebacktocontact}", Operator = "Equals", Right = "true" } });
        var report = new PackValidator().ValidateOne(tc);
