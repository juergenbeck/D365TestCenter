...
public sealed class DataverseService
{
    ...
    private const string TokenCacheFileName = "d365tc_integrationtests_cache.bin";

    // Environment aliases resolve via D365TC_ORG_<ALIAS> (e.g. D365TC_ORG_DEV); an absolute URL works directly.
    private const string EnvironmentVariablePrefix = "D365TC_ORG_";
    ...
    /// </summary>
    /// <param name="environment">Alias (z.B. "dev", aufgelöst über D365TC_ORG_DEV) oder absolute Org-URL</param>
    ...
    private static async Task RegisterTokenCacheAsync(IPublicClientApplication msalApp)
    {
        var cacheDir = Path.Combine(
            ...
            "D365TestCenter", "IntegrationTests");
    ...
    private static string ResolveEnvironmentUrl(string environment)
    {
        ...
        var url = Environment.GetEnvironmentVariable(EnvironmentVariablePrefix + environment.ToUpperInvariant());
        if (!string.IsNullOrWhiteSpace(url))
            return url;

        throw new ArgumentException(
            $"Unbekannte Umgebung: '{environment}'. Absolute Org-URL angeben oder {EnvironmentVariablePrefix}{environment.ToUpperInvariant()} setzen.");
    }
}
