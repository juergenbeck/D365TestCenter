...
public static class ResultSync
{
    ...
    /// <summary>
    /// Derives an env label from the org URL host, as a default for --env.
    /// "contoso-uat.crm4..." gives "uat": the suffix after the last hyphen of the
    /// first host label is used when it names an environment (contains dev, test,
    /// prod, uat, qa, stag or sandbox), so project-specific stages such as
    /// "-perftest" keep their own label. Otherwise a contains-match on test/prod/dev.
    /// </summary>
    static readonly string[] EnvWords = { "dev", "test", "prod", "uat", "qa", "stag", "sandbox" };
    ...
    public static string DeriveEnv(string org)
    {
        ...
        {
            var suffix = label[(dash + 1)..];
            if (suffix.All(char.IsLetterOrDigit) &&
                EnvWords.Any(w => suffix.Contains(w, StringComparison.Ordinal)))
                return suffix;
        }
