...
public static class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        // ADR 2026-09-16-1957: evidence documentation (screenshots with purpose + HTML/DOCX).
        runCommand.AddOption(new Option<string?>("--evidence-dir",
            "Directory for the evidence documentation of this run: screenshots, evidence.json, testdokumentation.html and testdokumentation.docx."));
        runCommand.Handler = CommandHandler.Create<string, string?, string?, string?, string?, bool, string, bool, string, string?, bool, string, string?, string?, string?, string?>(
            RunTests);
        rootCommand.AddCommand(runCommand);

        // ── evidence-report command (ADR 2026-09-16-1957) ────────
        var evidenceReportCommand = new Command("evidence-report",
            "Render testdokumentation.html and testdokumentation.docx again from an existing evidence.json (no Dataverse access).");
        evidenceReportCommand.AddOption(new Option<string>("--evidence-dir",
            "Evidence directory of a run (contains evidence.json and the images).") { IsRequired = true });
        evidenceReportCommand.Handler = CommandHandler.Create<string>(RenderEvidenceReport);
        rootCommand.AddCommand(evidenceReportCommand);
        ...
        syncDevOpsCommand.AddOption(new Option<string>("--devops-org",
            "Azure DevOps organisation, e.g. contoso.") { IsRequired = true });
        syncDevOpsCommand.AddOption(new Option<string>("--devops-project",
            "Azure DevOps project, e.g. CRM.") { IsRequired = true });
        ...
        var uiSetupCommand = new Command("ui-setup",
            "Create a Playwright storage-state by interactive login for non-production UI tests (hosts with -dev. or -test., more via --allow-env).");
        uiSetupCommand.AddOption(orgOption);
        uiSetupCommand.AddOption(new Option<string>("--output", () => "auth/storage-state.json",
            "Output path for the storage-state JSON"));
        uiSetupCommand.AddOption(new Option<string[]>("--allow-env",
            "Additional non-production environment marker, e.g. 'uat' for contoso-uat.crm4.dynamics.com (repeatable)."));
        uiSetupCommand.Handler = CommandHandler.Create<string, string, string[]>(
    ...
    static Task<int> RunTests(
        ...
        string? syncDefs, string? env, string? evidenceDir)
    {
        ...
        try
        {
            ...
            // ADR 2026-09-16-1957: optional evidence documentation.
            var evidence = !string.IsNullOrWhiteSpace(evidenceDir)
                ? new D365TestCenter.Cli.UiAutomation.EvidenceCollector(evidenceDir!)
                : null;
            if (evidence != null)
                Console.WriteLine($"  Testdokumentation: {evidence.Directory}");
            ...
            if (evidence != null)
            {
                ...
                try
                {
                    ...
                    EvidenceOutput.WriteRun(evidence, result, orchestrator.LastRunTestCases, org, envLabel, filter, Console.WriteLine);
                }
                catch (Exception docEx)
                {
                    // Documentation must not change the test exit code.
                    Console.WriteLine($"  Testdokumentation fehlgeschlagen: {docEx.Message}");
                }
            }
    ...
    static Task<int> RenderEvidenceReport(string evidenceDir)
    {
        try
        {
            var doc = EvidenceOutput.ReadJson(evidenceDir);
            var (html, docx) = EvidenceOutput.Render(evidenceDir, doc);
            Console.WriteLine($"  Testdokumentation neu erzeugt: {doc.TestCases.Count} Testfälle, {doc.TestCases.Sum(t => t.Items.Count)} Belege");
        ...
        bool scriptResults, string config)
    {
        try
        {
            ...
            // explicit --env we omit the field; a project wrapper can map org -> env.
    ...
    // Only one profile exists. Earlier project-specific profile names behaved identically to
    // "standard"; they are still accepted (with a notice) so existing wrapper scripts keep working.
    static ITestCenterConfig GetConfig(string name)
    {
        if (!string.Equals(name?.Trim(), "standard", StringComparison.OrdinalIgnoreCase))
            Console.Error.WriteLine(
                $"Hinweis: Config-Profil '{name}' ist entfallen, es gilt 'standard' (identisches Verhalten).");
