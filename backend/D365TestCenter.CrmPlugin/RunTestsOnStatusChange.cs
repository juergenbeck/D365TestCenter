...
public sealed class RunTestsOnStatusChange : IPlugin
{
    ...
    // der Depth hoch (verifiziert auf einer DEV-Org). Das bedeutet:
    ...
    // Backlog A (v5.3.14): 12 -> 5. Eine gemessene Plugin-Cascade hat rund 13 s/Test, womit 12er-Batches
    // das 2-Min-Sandbox-Timeout rissen (Lesson Learned 5). 5 x 13 s = 65 s < 100 s.
        ...
        try
        {
            ...
            if (testCases.Count == 0)
            {
                ...
                var emptyUpdate = new Entity(TestRunEntity, testRunId)
                {
                    ...
                    [FldTotal] = 0,
                    // Block 1 L (v5.3.11): Auch im Empty-Pfad jbe_fulllog
        ...
        Guid testRunId, List<TestCase> allTests, int offset, bool keepRecords)
    {
        ...
        if (isLastBatch)
        {
            ...
            // Block 1 L (v5.3.11): Plugin-Header + Per-Batch-Summary
